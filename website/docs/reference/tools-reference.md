...
| `skill_search` | Find skills by topic with local embeddings (offline, no API cost). Use when the skills index marks a category as "[count only]: N skills", or whenever it is unclear which skill covers a topic; returns names + one-line descriptions to load with skill_view(name). | — |
